...
describe("V0.5 Terry导演工作台", () => {
  ...
  it("单击任务只更新右侧只读信息栏，不进入编辑", async () => {
    ...
    expect(screen.queryByRole("dialog", { name: /编辑分镜/ })).not.toBeInTheDocument();
  });
  ...
  it("双击或右键任务进入同一套悬浮编辑窗", async () => {
    ...
    expect(screen.getByRole("dialog", { name: /编辑分镜 · T01-001/ })).toBeInTheDocument();
    ...
    expect(screen.getByRole("dialog", { name: /编辑分镜 · T01-002/ })).toBeInTheDocument();
  });
